...
You can refer this playlist for learning all the mathematical concepts related to Applied Linear Algebra, Probability Theory and Random Processes & Statistics which are required for understanding the ML Algorithms''')
        ...
        elif st.checkbox("Test your understanding in ML"):
            st.write("https://youtube.com/playlist?list=PLjfRmoYoxpNqmsxUe31RCtnDN8vmuLQW0");
...
In my this playlist , the important concepts of Digital Signal Processing  have been explained  with relevant  MATLAB simulations  .
                    
In the description of each videos , I have  posted the MATLAB codes , so that you can easily copy the code and run that in matlab.(If the code is not available in description , then it is posted in the comment section.)''')
...
Finger print scanners on all mobile phones, laptops.

Face Recognition software on all mobile phones, laptops.

All picture editors like Picasa, Prisma, MS Paint, Photoshop.

Finger print recognition used by Aadhar.

Face detection using any camera app on mobile phones to detect faces.''');
